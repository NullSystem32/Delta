class Container {
  ...
  static async refreshAll() {
    console.log('Refreshing all Liquid Glass instances...')
    Container.clearSnapshot()

    if (Container.instances.length === 0) return

    // Mark all as needing re-init
    Container.instances.forEach(inst => {
      inst.webglInitialized = false
      if (Container.waitingForSnapshot.indexOf(inst) === -1) {
        Container.waitingForSnapshot.push(inst)
      }
    })

    if (!Container.isCapturing) {
      Container.isCapturing = true
      // Use the first available instance to trigger capture
  ...
  init() {
    ...
    if (Container.pageSnapshot) {
      ...
      if (Container.waitingForSnapshot.indexOf(this) === -1) {
    ...
    html2canvas(document.body, {
      ...
      ignoreElements: function (element) {
        return (
          ...
          element.closest('[data-html2canvas-ignore]')
        )
      }
    })
      .then(snapshot => {
  ...
  initWebGL() {
    if (!Container.pageSnapshot || !this.gl || !this.active) return
  ...
  `
    ...
    void main() {
      ...
      vec2 center = vec2(0.5, 0.5);
        ...
        // Simple vertical gradient (Darkened)
        float gradientPosition = coord.y;
        vec3 topTint = vec3(0.05, 0.05, 0.05);
        vec3 bottomTint = vec3(0.01, 0.01, 0.01);
  ...
  startRenderLoop() {
    const render = () => {
      if (!this.active || !this.gl_refs.gl) return
